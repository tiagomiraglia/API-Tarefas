...
const prisma = new client_1.PrismaClient(); // Temporário até TypeScript Server recarregar
class KanbanController {
    // ABAS
    async listAbas(req, res) {
    ...
    async createCartao(req, res) {
        ...
        const userId = req.user?.id;
        if (!titulo || !coluna_id)
            return res.status(400).json({ message: 'Título e coluna obrigatórios' });
        try {
    ...
    async updateCartao(req, res) {
        const { id } = req.params;
        const { titulo, descricao, usuario_atribuido_id, data_vencimento, etiquetas, checklist, cor, status_atendimento, valor_orcamento, anotacoes } = req.body;
        const userId = req.user?.id;
        try {
    ...
    // VINCULAÇÃO WHATSAPP
    async vincularWhatsApp(req, res) {
    ...
    async desvincularWhatsApp(req, res) {
    ...
    async getMonitoramento(req, res) {
        const empresaId = Number(req.query.empresaId || req.user?.empresa_id);
        if (!empresaId || isNaN(empresaId)) {
        ...
        try {
            ...
            const usuariosComAcesso = usuariosComPermissoes.filter((u) => u.nivel === 'superadmin' || u.nivel === 'admin' || (u.permissoesKanban.abas && u.permissoesKanban.abas.length > 0)).length;
            res.json({
                estrutura: abas,
                usuarios: usuariosComPermissoes,
                estatisticas: {
                    totalAbas: abas.length,
                    totalColunas: abas.reduce((acc, aba) => acc + aba.colunas.length, 0),
                    totalCartoes,
                    totalUsuarios,
                    usuariosComAcesso
                }
