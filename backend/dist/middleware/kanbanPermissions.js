...
exports.verificarPermissaoAba = verificarPermissaoAba;
exports.verificarPermissaoColuna = verificarPermissaoColuna;
exports.verificarPermissaoEdicao = verificarPermissaoEdicao;
exports.verificarPermissaoMover = verificarPermissaoMover;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
// Verifica se o usuário tem permissão para acessar uma aba
async function verificarPermissaoAba(req, res, next) {
...
async function verificarPermissaoColuna(req, res, next) {
    const userId = req.user?.id;
    ...
    try {
        ...
        req.kanbanPermissao = permissaoColuna || permissaoAba;
        next();
    }
    catch (error) {
        console.error('Erro ao verificar permissão:', error);
...
async function verificarPermissaoEdicao(req, res, next) {
    const permissao = req.kanbanPermissao;
...
async function verificarPermissaoMover(req, res, next) {
    const permissao = req.kanbanPermissao;
    const usuario = req.user;
    if (usuario?.is_superuser || usuario?.nivel === 'admin') {
        return next();
...
//# sourceMappingURL=kanbanPermissions.js.map