...
router.post('/', authenticateJWT, async (req: AuthRequest, res) => {
  try {
    ...
    await Promise.all(avisoIds.map(async (aviso_id: number) => {
...
router.get('/:avisoId', authenticateJWT, async (req: AuthRequest, res) => {
  try {
    const avisoId = Number(req.params.avisoId);
    if (!avisoId) return res.status(400).json({ error: 'ID inválido' });
