...
export class WhatsAppKanbanController {
  private readonly WPPCONNECT_URL = process.env.WPPCONNECT_URL || 'http://localhost:5000';

  // Status da conexão WhatsApp
  async getStatus(req: AuthRequest, res: Response) {
    try {
      const response = await axios.get(`${this.WPPCONNECT_URL}/status/default`, { timeout: 3000 });
  ...
  async connect(req: AuthRequest, res: Response) {
    try {
      const response = await axios.post(`${this.WPPCONNECT_URL}/connect/default`, {}, { timeout: 5000 });
  ...
  // Reconectar WhatsApp
  async reconnect(req: AuthRequest, res: Response) {
    try {
      const response = await axios.post(`${this.WPPCONNECT_URL}/reconnect`, {}, { timeout: 5000 });
      res.json(response.data);
    } catch (error: any) {
      res.status(500).json({ 
  ...
  async cleanSession(req: AuthRequest, res: Response) {
    try {
      const response = await axios.post(`${this.WPPCONNECT_URL}/clean-session`, {}, { timeout: 10000 });
      res.json(response.data);
    } catch (error: any) {
      res.status(500).json({ 
  ...
  async getConversas(req: AuthRequest, res: Response) {
    try {
      // Verifica se WhatsApp está conectado
      let statusResponse;
      try {
        statusResponse = await axios.get(`${this.WPPCONNECT_URL}/status/default`, { timeout: 3000 });
      } catch (error: any) {
      ...
      const response = await axios.get(`${this.WPPCONNECT_URL}/api/default/all-chats`, { timeout: 10000 });
      ...
      const conversas = response.data.map((chat: any) => {
      ...
      conversas.sort((a: any, b: any) => b.timestamp - a.timestamp);
  ...
  async getMensagens(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      const response = await axios.get(`${this.WPPCONNECT_URL}/api/default/all-messages/${encodeURIComponent(conversaId)}`);
      
      if (!response.data || !Array.isArray(response.data)) {
        return res.json([]);
      }

      const mensagens = response.data.map((msg: any) => ({
  ...
  async enviarMensagem(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      const response = await axios.post(`${this.WPPCONNECT_URL}/api/default/send-message`, {
        phone,
        message
      });
  ...
  async marcarComoLido(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      const response = await axios.post(`${this.WPPCONNECT_URL}/api/default/mark-as-read`, {
        chatId
      });
  ...
  async sincronizarMensagens(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      const response = await axios.get(`${this.WPPCONNECT_URL}/api/default/all-messages/${cartaoWhatsApp.conversa_id}`);
  ...
  async webhookMensagem(req: Request, res: Response) {
  ...
  async criarCartao(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      if (colunaId) {
        ...
        coluna = await (prisma as any).coluna.findFirst({
          where: {
            id: colunaId,
            aba: {
      ...
      const cartao = await (prisma as any).cartao.create({
        data: cartaoData
      });
      ...
      const cartaoWhatsApp = await (prisma as any).cartaoWhatsApp.create({
        data: {
          cartao_id: cartao.id,
  ...
  async importarHistorico(req: AuthRequest, res: Response) {
    ...
    try {
      ...
      const coluna = await (prisma as any).coluna.findFirst({
        where: {
          recebe_whats: true,
          aba: {
            empresa_id: usuario.empresa_id
      ...
      const response = await axios.get(`${this.WPPCONNECT_URL}/api/default/all-chats`);
      ...
      for (const chat of response.data) {
        ...
        const existente = await (prisma as any).cartaoWhatsApp.findFirst({
        ...
        try {
          // Busca mensagens
          const msgResponse = await axios.get(`${this.WPPCONNECT_URL}/api/default/all-messages/${encodeURIComponent(chatId)}`);
          ...
          if (mensagens.length > 0) {
            ...
            for (const msg of mensagensParaSalvar) {
              try {
                await (prisma as any).mensagemWhatsApp.create({
                  data: {
                    cartao_whatsapp_id: cartaoWhatsApp.id,
  ...
  private async getNextPositionInColumn(colunaId: number): Promise<number> {
    const lastCartao = await (prisma as any).cartao.findFirst({
