...
export class UserController {
  ...
  async updateUser(req: Request & { files?: any }, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      if (req.files && (req.files as any).foto) {
        const file = (req.files as any).foto[0];
      ...
      if (permissoes) {
        ...
        if (typeof permissoes === 'string') {
          try { permissoes = JSON.parse(permissoes); } catch {}
        }
      ...
      values.push(id);
      await pool.query(`UPDATE "Usuario" SET ${setClause} WHERE id = $${fields.length + 1}`, values);
  ...
  async deleteUser(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      if (isSuperuser) {
        ...
        const countResult = await pool.query('SELECT COUNT(*) FROM "Usuario" WHERE is_superuser = true');
        ...
        const adminsResult = await pool.query('SELECT COUNT(*) FROM "Usuario" WHERE empresa_id = $1 AND nivel = $2', [empresaIdOfUser, 'admin']);
  ...
  async reactivateUser(req: Request, res: Response): Promise<void> {
    try {
      const { id } = req.params;
  const result = await pool.query('UPDATE "Usuario" SET suspenso = false WHERE id = $1 RETURNING id, nome, email, suspenso', [id]);
  ...
  async getUserById(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await pool.query(
        `SELECT id, nome, email, foto, permissoes,
  ...
  async createTeamUser(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const exists = await pool.query('SELECT id FROM "Usuario" WHERE email = $1 AND empresa_id = $2', [email, empresaId]);
      ...
      `;
      await sendMail(email, 'Acesso ao sistema - senha temporária', html);
      res.status(201).json({
  ...
  async resendTempPassword(req: AuthRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      ...
      `;
      await sendMail(emailToSend, 'Nova senha temporária de acesso', html);
